...
find . -name template.yaml | while read -r fname; do
  ...
  sam package --template-file "$templateName" --output-template-file "$outputFileName" --s3-bucket "orders-demo-configs-$1"
done

echo ""
echo "Coping to s3 templates"
aws s3 cp --recursive --exclude "*template.yaml" templates "s3://orders-demo-configs-$1/templates/"
